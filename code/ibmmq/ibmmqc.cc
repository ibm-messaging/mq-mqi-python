...
/* ----------------------------------------------------------------------- */
/* This is the only part that's in C++ to make it easy to use the map      */
/* construct. The map here stashes information about the buffer used in    */
/* MQGET. The original code did a malloc/free for every MQGET; now we can  */
/* malloc once and reuse the same buffer for each MQGET to the same hObj.  */
/* with a realloc if necessary. The buffers are then freed during MQCLOSE  */
/* and MQDISC.                                                             */
/* ----------------------------------------------------------------------- */
using namespace std;

#define HOBJ_WILDCARD (-999)
...
static map<string, getBuffer *> getBufferMap;
...
string makeKey(MQHCONN hConn, MQHOBJ hObj) {
  string s;
  if (hObj == HOBJ_WILDCARD) { // special case to match all
    s = to_string(hConn) + "/";
  } else {
    s = to_string(hConn) + "/" + to_string(hObj);
  }
  debug(1,"makeKey: returning %s",s.c_str());
  return s;
}
...
static char * updateQ(MQHCONN hConn, MQHOBJ hObj, size_t size) {
    ...
    int mapCount = getBufferMap.count(key);
    if (mapCount == 0) {
        debug(1,"Allocating new buffer of size %d",size);
        auto bd = (getBuffer *)myAlloc(sizeof(getBuffer),"buffer descriptor");
        if (bd) {
        ...
        auto bd = getBufferMap[key];

        if (bd->size < size) {
            debug(1,"Reallocating new buffer from size %d to %d",bd->size, size);
            ...
            debug(1,"Reusing buffer of size %d at %p",size,bd->buf);
            buf = bd->buf;
        }
    }
...
static void deleteQ(MQHCONN hConn, MQHOBJ hObj) {
    ...
    if (mapCount == 1) {
        debug(1,"DeleteQ: found entry for %d",hObj);

        auto bd = getBufferMap[key];
        myFree(bd->buf);
        myFree(bd);
        getBufferMap.erase(key);
    } else {
        debug(1,"DeleteQ: no entry for %d",hObj);
...
static void deleteAll(MQHCONN hConn) {
  ...
  for (auto it = getBufferMap.begin(); it != getBufferMap.end();) {
    string mapKey = it->first;

    if (mapKey.find(key, 0) == 0) { // startswith()
      size_t idx = mapKey.find("/", 0);

      auto bd = getBufferMap[mapKey];
       debug(1, "  ... removing Q %s with %p",mapKey.c_str(),bd->buf);
       myFree(bd->buf);
       myFree(bd);
       it = getBufferMap.erase(it);
    }
  }
  unlock();
}
...
/* Some failures can still mean a resource has closed/delete OK */
...
static PyObject * ibmmqc_MQCONNX(PyObject *self, PyObject *args) {
  ...
  if(mqcd) {
    ...
    // Until V10, SPLProtection was the only MQCD field more recent than the baseline 9.1
    // ... and it's only really valid for qmgr channels so should never be set by
    // client applications. But it was convenient for testing the VERSION processing.
...
static PyObject * ibmmqc_MQCB(PyObject *self, PyObject *args) {
  ...
  cbd->CallbackFunction = (MQPTR)cbFunc_C;
